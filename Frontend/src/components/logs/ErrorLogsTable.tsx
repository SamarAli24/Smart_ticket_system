...
const columns = ["Method", "Path", "Status", "Message", "Exception", "Timestamp"];

interface ErrorLogsTableProps {
  logs: ErrorLogDto[];
  onViewDetails: (log: ErrorLogDto) => void;
}

export default function ErrorLogsTable({ logs, onViewDetails }: ErrorLogsTableProps) {
  return (
            ...
            return (
              ...
              >
                ...
                <td className="max-w-[180px] px-5 py-4 text-slate-500">
                  <p className="truncate">{log.exceptionType ?? "-"}</p>
                </td>
